...
FastAPI application with SQLAlchemy, Alembic migrations, and SQLite (`education.db`) for the POC.
...
- **SQLAlchemy 2 (async)** — ORM; SQLite via `aiosqlite`
- **Alembic** — schema migrations under `alembic/`
- **JWT** — access + refresh tokens (`python-jose`)
- **Redis + ARQ** — async PDF ingest workers (`modules/rag`, `workers/`)
- **sqlite-vec** — sidecar vector index (`education_vectors.db`)
...
PDF ingest requires Redis and an ARQ worker process:
...
uv run arq education_platform.workers.settings.WorkerSettings
```

First Docling / sentence-transformers runs download models (can take several minutes). Uploads are stored under `backend/var/uploads/`; vectors live in `backend/education_vectors.db`.
...
| `modules/rag` | Knowledge-document upload/status, local blobs, chunking, embeddings, sqlite-vec |
| `modules/assessments` | Quiz attempts, scoring (no answer keys on student responses) |
| `workers` | ARQ ingest job: Docling parse → chunk → embed → index |
...
| `DATABASE_URL` | `backend/education.db` | SQLite connection |
...
| `VECTOR_DB_PATH` | `backend/education_vectors.db` | sqlite-vec sidecar |
